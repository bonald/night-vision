...
let throttleDelay = 100; // Set delay as you need
...
export default class Input {
  ...
  // check if any computation could be removed or optimized
  touch2mouse(e) {
    this.calcOffset(); //see if this is needed for every event
    return {
      original: e.srcEvent,
  ...
  panFade(event) {
    ...
    this.fade = requestAnimationFrame(
      function fade() {
        v *= 0.85;
        if (Math.abs(v) < v0) {
          ...
          this.range[1] += v;
          this.changeRange();
          this.fade = requestAnimationFrame(fade.bind(this));
        }
      }.bind(this)
    );
  }
  ...
  pinchZoom(scale) {
    this.range[0] = this.pinch.r[0] - (this.pinch.t / scale - this.pinch.t) * 0.5;
    this.range[1] = this.pinch.r[1] + ((this.pinch.t * 1) / scale - this.pinch.t) * 0.5;
    this.changeRange();
  }
